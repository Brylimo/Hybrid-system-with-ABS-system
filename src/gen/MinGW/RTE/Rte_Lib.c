/** @file         Rte_Lib.c
  ...
  *
  * @note         RTE_WOWP_EVENTS                        = 13
  *               RTE_NUM_ALARMS                         = 0
  *               RTE_NUM_TASKS                          = 25
  *               RTE_TASKS_ARRAYSIZE                    = 25
  *               RTE_MSITABLE_SIZE                      = 0
  *               RTE_NULL_SCHEDULE                      = 0
  *               RTE_OS_EVENTS                          = 1
...
Rte_DecrementWaitingCount( P2VAR(Rte_WaitableDatum, AUTOMATIC, RTE_DATA) datum,
                           ...
                           Rte_EventType event )
{
   *Rte_Tasks[idx].waitingEv = RTE_WOWP_EVENTS;
   if ( 0 < --datum->count )
   {
      ...
      for ( idx = 0; ( !found ) && ( idx < RTE_TASKS_ARRAYSIZE ); ++idx )
      {
         P2CONST(Rte_EventType, AUTOMATIC, RTE_DATA)  evPtr = Rte_Tasks[idx].waitingEv;
         if ( (evPtr) && (*evPtr == event) )
         {
            datum->firstWaitingTask = idx;
            found = TRUE;
         }
      }
   }
              ...
              Rte_EventType event )
{
   ...
   else
   {
      ...
      if ( datum->count )
      {
         ...
         /* The following variable name is chosen to avoid hiding the `index'
          * non-ISO C alias of the strchr function that is present in many
          * standard library implementations, since that can result in compiler
          * warnings.
          */
         Rte_TaskArrayIndex   idx = datum->firstWaitingTask;
         TaskType tref = RTE_TASK_FROM_REF( Rte_Tasks[idx].task );
         ...
         Rte_DecrementWaitingCount( datum, idx, event );
      }
      else
      {
         datum->pending = 1;
                     ...
                     const TickType timeout )
{
   ...
   /* The following variable name is chosen to avoid hiding the `index' non-ISO
    * C alias of the strchr function that is present in many standard library
    * implementations, since that can result in compiler warnings.
    */
   Rte_TaskArrayIndex   idx = Rte_GetCurrentTaskIndex( &task );    /* task gets initialised during this call */

   if ( idx == RTE_TASKS_ARRAYSIZE )
   {
      /*lint -e(904) Allow early return */
      return E_NOT_OK;
   }
   ...
   {
      alarm = RTE_ALARM_FROM_INDEX( Rte_Tasks[idx].alarm );
